...
# Task 1: To compare similarity within invivo sequences 

invivo<- fread("S6_invivo.csv", sep = ",", header = TRUE)


#make a new data table that will have identifier and its sequence next to each other. 
#Transform invivo table 


reshaped_invivo <- data.frame(
  Identifier = invivo[seq(1, nrow(invivo), 2), 1], # Extract odd rows as identifiers
  Sequence = invivo[seq(2, nrow(invivo), 2), 1]   # Extract even rows as sequence
)

colnames(reshaped_invivo)<- c("invivo_identifier", "invivo_sequences")

reshaped_invivo$length<- nchar(reshaped_invivo$invivo_sequences)
...
identifier_list <- list(reshaped_invivo$filename) #if list is removed it will save as values with 516 strings.
...
colnames(test)<- "invivo_sequences"
...
fwrite(similarity_matrix,
       "invivo_sequences_similarity_matrix_without_gaps.csv")

# if you plan to read the similarity table again then read as data as data frame 


similarity_matrix<- fread("invivo_sequences_similarity_matrix_without_gaps.csv", header= TRUE, sep = ",")
...
fwrite(similarity_matrix_new,
       "invivo_sequences_without_gaps_similarity_matrix_heatmap_input.csv", sep = ",")
...
fwrite(distance_matrix, "invivo_sequences_without_gaps_similarity_matrix_heatmap_distance_input.csv")
...
invivo_sequence_heatmap_without_gaps<- pheatmap(
...
invivo_sequence_heatmap_without_gaps_using_distance<- pheatmap(
  ...
  scale = "none",                    
  main = "Comparative Analysis of Sequence Similarity distance in invivo_sequences")
...
sorted_data<- similarity_matrix[invivo_sequence_heatmap_without_gaps$tree_row$order,] # This reorders the rows of similarity_matrix according to the clustering order

#The first column will provide the sequence identifier name based on first column of sorted data 
heatmap_order<- sorted_data %>% select("invivo_sequences") #captures first row

sorted_data4 <-similarity_matrix[invivo_sequence_heatmap_without_gaps$tree_row$order, #This reorders the rows of similarity_matrix according to the clustering order
                                 sorted_data$invivo_sequences]#reorder the columns based on invivo_sequences
...
heatmap_order2_invivo <- heatmap_order %>% select(original_row_order, serial_row_number, invivo_sequences, 1:90)
